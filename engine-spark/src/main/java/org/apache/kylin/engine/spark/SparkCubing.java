...
public class SparkCubing extends AbstractApplication {
    ...
    private static final Option OPTION_COPROCESSOR = OptionBuilder.withArgName("coprocessor").hasArg().isRequired(true).withDescription("Coprocessor Jar Path").create("coprocessor");//覆盖协处理器的jar包
    ...
    //向spark的环境中加载confPath文件下的xml和properties文件
    ...
    protected void execute(OptionsHelper optionsHelper) throws Exception {
        final String hiveTable = optionsHelper.getOptionValue(OPTION_INPUT_PATH);//hive的表名字作为输入源
        ...
        final DataFrame intermediateTable = sqlContext.sql("select * from " + hiveTable);//查询hive的sql
        ...
        final KylinConfig kylinConfig = KylinConfig.getInstanceFromEnv();
        kylinConfig.overrideCoprocessorLocalJar(coprocessor);//覆盖协处理器的jar包

        setupClasspath(sc, confPath);//向spark的环境中加载confPath文件下的xml和properties文件
        intermediateTable.cache();
        writeDictionary(intermediateTable, cubeName, segmentId);
        final JavaRDD<List<String>> rowJavaRDD = intermediateTable.javaRDD().map(new org.apache.spark.api.java.function.Function<Row, List<String>>() {//处理每一行数据--转换成列的集合
            @Override
            public List<String> call(Row v1) throws Exception {//参数是每一行数据
